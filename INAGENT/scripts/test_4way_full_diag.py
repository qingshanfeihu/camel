...
_CLI_PDF_PATH = _REFERENCE / "cli_1-82.json"
_APP_PDF_PATH = _REFERENCE / "app_1-40.json"
...
def _graphrag_search(query: str, keywords: List[str]) -> Dict[str, Any]:
    ...
    for kw in keywords:
        ...
        for i, t in enumerate(titles):
            if kw_lower in str(t) or kw_lower in str(descs[i]):
...
def _vector_search(query: str, top_k: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        # Relevance check: any query keyword appears in top snippet
        q_words = [w.lower() for w in query.split() if len(w) > 1]
        top_text = (result["top3_snippets"][0] if result["top3_snippets"] else "").lower()
        result["relevant"] = result["hit"] and any(w in top_text for w in q_words)
    except Exception as exc:
...
def run_cli_4way(ct_data: list, cli_pdf: list, count: int, seed: int):
    ...
    for e in cli_pdf:
        ...
        if re.match(r'^[a-z]', title) and len(title.split()) <= 6:
            cmd_like.append(e)
        elif re.search(r'[a-zA-Z]{2,}', title):
            desc_like.append(e)
    candidates = cmd_like + desc_like
    random.seed(seed)
...
def _cli_leaf_association(section_title: str, product_module: str,
    ...
    for entry in ct_data:
        ...
        mod = m.get("product_module", "").lower()

        hits = sum(1 for kw in kws if kw in pc or kw in mod)
...
def run_app_4way(app_data: list, ct_data: list, count: int, seed: int):
    candidates = [e for e in app_data
                  if len(e.get("page_content", "")) > 80
                  and e.get("metadata", {}).get("section_title", "")
                  and e.get("metadata", {}).get("section_title", "") != "⽬录"]
    random.seed(seed)
...
def print_app_report(results: list):
    ...
    for i, r in enumerate(results):
        ...
        print("  %d. [%-25s] mod=%-10s" % (i + 1, r["section"][:25], (r["module"] or "?")[:10]))
    ...
    print()
    print("  ─── 汇总 ───")
    print("  S1 app.pdf 原文:         %d/%d (%d%%)" % (totals["s1"], n, 100*totals["s1"]//n))
    print("  S2 CLI叶子关联:          %d/%d (%d%%)" % (totals["s2"], n, 100*totals["s2"]//n))
    print("  S3 向量(命中):           %d/%d (%d%%)" % (totals["s3"], n, 100*totals["s3"]//n))
    print("  S3 向量(相关命中):       %d/%d (%d%%)" % (totals["s3r"], n, 100*totals["s3r"]//n))
    print("  S4 GraphRAG:             %d/%d (%d%%)" % (totals["s4"], n, 100*totals["s4"]//n))
    print("=" * 90)
    return totals
...
def main():
    ...
    cli_pdf = _load_json(_CLI_PDF_PATH)
    app_data = _load_json(_APP_PDF_PATH)
    kb = _load_json(_KB_PATH)

    logger.info("CommandTree: %d, CLI PDF: %d, APP PDF: %d, KB: %d",
                len(ct_data), len(cli_pdf), len(app_data), len(kb))
