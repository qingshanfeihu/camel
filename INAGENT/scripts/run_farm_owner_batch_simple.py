...
for batch_dir in batches:
    ...
    try:
        ...
        if decisions_src.exists():
            ...
            print(f"[INFO] 已输出 decisions 文件")

        proposals_src = TEMP_REFERENCE_DIR / "_quality_rule_proposals.json"
        if proposals_src.exists():
            proposals_dst = output_batch_dir / "_quality_rule_proposals.json"
            shutil.copy2(proposals_src, proposals_dst)
            print(f"[INFO] 已输出质检规则建议文件")
...
print(f"[INFO] 结果已输出到: {FARM_OWNER_OUTPUT_ROOT}")
