...
| **运行时查询** | [cli_graph_store.py](INAGENT/rag/cli_graph_store.py) `CLIGraphStore` | `command_exists(cmd) -> Tuple[bool, List[str]]`（农场主 `_query_tree_context` 解包）；以及 `extract_subgraph`、`get_branch_ids`、`derive_trunk`、`l2_connected`、`get_operations_group` 等 | 上层按 **模块 hint、branch、关键词** 取子图或连通性；**mock 须返回二元组**，勿只返回 `bool` |
